...
image = cv2.imread(image_path)  # OpenCV for image reading

# Convert image to RGB for processing (OpenCV uses BGR by default)
...
# Load YOLO model
model = YOLO('/content/drive/MyDrive/best_p6.pt')  # Specify the path to your model file
...
agent_positions = [(np.random.randint(0, image_rgb.shape[1]), np.random.randint(0, image_rgb.shape[0])) for _ in range(num_agents)]
...
ub = [image_rgb.shape[1], image_rgb.shape[0]] * num_agents  # Upper bounds for agent positions
